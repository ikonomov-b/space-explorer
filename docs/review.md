...
**Status:** Open. No decision records the owner's intent yet; clearing it requires one that fixes which view each level is judged in, whether a system view changes the close-views clause of decision 0045, the order of the viewer and the gate against these iterations, and where a graphic iteration's sample is kept.
