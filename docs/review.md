...
| 50 | [A stored quantity carries no unit, no default, and no validation limit](#50-a-stored-quantity-carries-no-unit-no-default-and-no-validation-limit) | Foundational | **Open**; registry revision 4 specified by [decision 0060](decisions/0060-registry-revision-4-units-defaults-and-validation-limits.md), clearing when it is built |
...
| 53 | [A barycentre holds nothing, so a published pair contradicts its own masses](#53-a-barycentre-holds-nothing-so-a-published-pair-contradicts-its-own-masses) | Foundational | **Open**; its registry half is specified by [decision 0060](decisions/0060-registry-revision-4-units-defaults-and-validation-limits.md), its placement rule waits on grammar version 5 |
...
**Status:** Open, with the format specified. The owner chose the fuller option on 2026-09-10 — unit, default, and the per-category validation limits, rather than striking validation limits from decision 0035 and the glossary — and [decision 0060](decisions/0060-registry-revision-4-units-defaults-and-validation-limits.md) specifies registry revision 4 precisely enough to build from: a closed unit vocabulary with a decimal exponent beside the fraction bits, a unit for every parameter of revision 3, a default present exactly for the kinds that admit one, and the decoder's own bounds moved out of C# constants into the record's header. The finding clears when the revision is built, not when the record is accepted. One quantity it had to decide rather than record, `artifact-part.mass` in grams, is itself the finding's evidence.
