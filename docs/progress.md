...
| M0a: primitive-set foundation | **In progress.** Of the plan's sixteen exit criteria below, two are done, four are partial, and ten are not started. The frozen random foundation is not itself one of the plan's M0a criteria; it appears under Determinism in the verification table. The canonical encoding and content hash appear in criterion 5. |
...
| 3 | Validate, store, reload, and query sets without rerolling | **Partial.** `CanonicalReader` reads back everything `CanonicalWriter` writes — every fixed- and variable-width field, text, length-prefixed framing, `ContentHash`, and `PackId` — and round-trips the exact vector [decision 0020](decisions/0020-canonical-encoding-and-content-hash.md) froze, closing the gap that decision named ("the format is written and hashed but never parsed back"). Mutation-checked on four rules: reading fixed-width values in the wrong byte order, dropping the zig-zag sign step, admitting an overlong variable-width encoding, and skipping the domain check, each of which turns the suite red. Not held: there is still no set specification to validate, store, reload, or query, because none exists ([decision 0022](decisions/0022-p0-gates-the-first-frozen-content-format.md)). |
...
| 11 | Minimal Godot viewer | Not started. `Main.tscn` holds the exported-build smoke entry point plus a materials/environment preview: a `WorldEnvironment` on the airless starfield sky, a directional light, one sphere per surface material (rocky, icy, vegetated), a bumpy ground patch under the rocky sphere from a procedural vertex-displacement shader, and two cameras: a diorama overview and a ground-level surface view standing on the rocky field, the latter active by default. This previews hand-authored placeholder assets for visual review, not the [decision 0002](decisions/0002-primitive-set-content-gate.md) content-gate viewer, which renders generated primitives from a pipeline that does not exist yet, and the ground patch is a shader-displaced plane, not the [decision 0001](decisions/0001-materialize-authoritative-terrain.md) authoritative heightfield, which belongs to the still-not-started M0b milestone. |
