...
| M0a: primitive-set foundation | **In progress.** Of the plan's sixteen exit criteria below, one is done, five are partial, and ten are not started. The frozen random foundation is not itself one of the plan's M0a criteria; it appears under Determinism in the verification table. The canonical encoding and content hash appear in criterion 5. |
...
| 12 | Native packaged smoke tests | **Partial.** `smoke.sh` and `smoke.ps1` exist and now run as CI steps on `ubuntu-latest` and `windows-latest`, which download and cache the official Godot 4.7.2 mono editor and export templates per OS before exporting and running the smoke check ([workflow](../.github/workflows/ci.yml)). Verified locally on the Debian 13 workstation, from a freshly downloaded (not the pre-installed) Godot binary: `SMOKE OK`. The actual `ubuntu-latest`/`windows-latest` run result is pending the next push; this row is updated once that lands. |
...
| Exported-build smoke tests in continuous integration | Wired: the [workflow](../.github/workflows/ci.yml) downloads and caches Godot 4.7.2 mono and its export templates from the official release per OS, then runs `smoke.sh`/`smoke.ps1`. Awaits confirmation from the first `ubuntu-latest`/`windows-latest` run before this closes; see M0a exit criterion 12. |
