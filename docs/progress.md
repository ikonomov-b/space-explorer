...
Complete and self-consistent as of 2026-09-07: the nineteen documentation-stage [review](review.md) findings are cleared, twenty [decision records](decisions/README.md) are accepted, and every internal link and anchor resolves. The implementation review of the same day raised [findings 20 to 27](review.md#findings-raised-by-the-implementation-review): two are cleared, including the stream-derivation hash, now generator version 2; six are open, one of them foundational, the mutable random-stream struct. Apart from the random foundation and the canonical encoding, the design statements are untested against code.
