...
| M0a: primitive-set foundation | **In progress.** Of the plan's exit criteria below, one is done, three are partial, and twelve are not started. The frozen random foundation is not itself one of the plan's M0a criteria; it appears under Determinism in the verification table. The canonical encoding and content hash appear in criterion 5. |
...
| 15 | Define the core-owned transport contract, commands, and join handshake, tested over an in-memory transport in continuous integration | Not started. No transport, command, or handshake code exists ([decision 0027](decisions/0027-in-memory-transport-commands-and-handshake-at-m0a.md)). |
...
| Multiplayer | **Not started.** No transport, command, or handshake code exists; the plan runs this suite over the in-memory transport from M0a onward, now M0a exit criterion 15 ([decision 0027](decisions/0027-in-memory-transport-commands-and-handshake-at-m0a.md)). |
...
Complete and self-consistent as of 2026-09-07: the nineteen documentation-stage [review](review.md) findings are cleared, twenty-seven [decision records](decisions/README.md) are accepted, and every internal link and anchor resolves. The implementation review of the same day raised [findings 20 to 27](review.md#findings-raised-by-the-implementation-review), all cleared: one by a technical-design change and seven by decisions 0021 to 0024; decision 0025 settles the P0 assessment decision 0022 reserved. A scan of the development plan raised [findings 28 and 29](review.md#findings-raised-by-the-development-plan-scan), cleared by decisions 0026 and 0027. Apart from the random foundation and the canonical encoding, the design statements are untested against code.
